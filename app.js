...
async function initApp() {
    ...
    if (config.activeDate) {
        ...
        if (diffTime >= 0) {
            ...
            realYear = Math.floor(diffDays / 365) + 1;
            daysLeft = 365 - (diffDays % 365);
        }
    }
    if (!config.activeYear || config.activeYear > config.policyTerm) config.activeYear = realYear; // Default view is actual year
    ...
    document.querySelectorAll('.tableYearDisplay').forEach(el => el.innerText = config.activeYear);
...
async function calculateTotalsForActiveYear() {
    usedTotals = { 'Total': 0, 'Critical': 0, 'HospitalDays': 0, 'HospitalBenefit': 0, 'HospitalRoomValue': 0, 'HospitalRoomDays': 0 };
    usedInner = {};
...
function renderProgressBars() {
    ...
    let tRem = config.baseCover - usedTotals['Total'];
    let tPct = Math.min((usedTotals['Total'] / config.baseCover) * 100, 100);
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 border border-brand-200 bg-brand-50 rounded-lg">
                        <div class="flex justify-between items-end mb-1">
                            <span class="font-semibold text-slate-800"><i class="fa-solid fa-umbrella text-brand-600"></i> Overall Policy Cover</span>
                            <span class="text-xs text-slate-500 font-bold max-w-[50%] text-right truncate" title="Max: ${formatRs(config.baseCover)}">Max: ${formatRs(config.baseCover)}</span>
                        </div>
...
async function renderClaimsTable() {
    ...
    tableBody.innerHTML = '';
    const claims = await db.claims.where('policyYear').equals(Number(config.activeYear)).reverse().sortBy('date');
...
document.getElementById('claimForm').addEventListener('submit', async (e) => {
    ...
    if (existingOldRec && existingOldRec.policyYear === Number(config.activeYear)) {
        ...
        else if (existingOldRec.category === 'Hospital Room') validateTotals['Total'] -= oA;
        else {
    ...
    if (category === 'Critical Illness') {
        ...
        // Validate Overall Policy Cover
        if (amountToDeduct > (config.baseCover - validateTotals['Total'])) return limError(config.baseCover - validateTotals['Total']);
    ...
    try {
        ...
        if (file) {
            ...
            reader.onload = async () => {
                ...
                if (editingClaimId) await db.claims.update(editingClaimId, payload);
                else await db.claims.add(payload);
                saveSuccess();
            };
...
window.addEventListener('DOMContentLoaded', checkFirstTimeSync);